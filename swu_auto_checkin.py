...
# ==================== 配置常量 ====================
MANUAL_TOKEN = ""
CHECKIN_TIME_RANGE = ["21:00", "23:30"]          # 固定时间，不随机
MAX_RETRIES = 3
LOGIN_URL = (
...
def browser_page(headless: bool = False) -> Generator[ChromiumPage, None, None]:
    """创建浏览器页面（上下文管理器），用于自动登录时的临时使用，但为了返回page我们改用手动管理，
       该函数保留仅供内部调用，实际登录函数会自行创建并返回page"""
...
# ==================== 登录主函数（修改为返回 page 和 token） ====================
def login_and_get_page(username: str, password: str, headless: bool = False) -> Tuple[ChromiumPage, str]:
    """
    使用文件一的登录逻辑，但返回浏览器页面对象和 token
    调用者需负责最终关闭 page
    """
    last_exception = None
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        try:
            ...
            print("等待 2 秒后重试...")
            time.sleep(2)
    raise RuntimeError(f"登录失败，已重试 {MAX_RETRIES} 次。") from last_exception
...
def checkin_with_page(page: ChromiumPage, token: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        payload = {
            ...
            "xh": student_id,
            "qdsj": CHECKIN_TIME_RANGE,       # 固定时间，不随机
        }
...
# ==================== 主程序 ====================
def main():
    parser = argparse.ArgumentParser(description='西南大学自动打卡（合并版）')
    ...
    if page is not None:
        # 优先使用浏览器打卡
        success, reason = checkin_with_page(page, token)
        if not success:
            print(f"⚠️ 浏览器打卡失败（{reason}），切换到 requests 备用方式...")
            success, reason = checkin_with_requests(token)
    else:
        # 手动 token 时直接使用 requests 方式
        success, reason = checkin_with_requests(token)
