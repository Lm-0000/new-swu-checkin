该方法使用drissionpag库、基于模拟人在浏览器登录从而获取token，实现swu钉钉寝室自动打卡，在获取token部分可能失败，希望有更有能力者开发新的技术路线。 
...
警告：如有基于我方法中获取token板块、使用的那一长串网址的方法，请勿公开传播（那一长串网址，你正常途径是拿不到的，是不是一看就知道了）。
